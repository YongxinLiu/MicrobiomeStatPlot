...
参考：https://mp.weixin.qq.com/s/78ncCvwVfiDhG9tjZmDuMA
...
这是来自于加州大学洛杉矶分校的Emeran A. Mayer团队在2023年发表于Microbiome上的一篇论文用到的dbRDA图。论文题目为：Multi-omics profles of the intestinal microbiome in irritable bowel syndrome and its bowel habit subtypes

This is a dbRDA plot used in a paper published in Microbiome in 2023 by Emeran A. Mayer's team from UCLA. The paper is titled: Multi-omics profiles of the intestinal microbiome in irritable bowel syndrome and its bowel habit subtypes
...
dbRDA plot using R software

微生物物种
Microbiota species

```{r species, include=TRUE}
#load packages
#rm(list = ls())
#setwd('C:/users/usersname/desktop')
library(vegan)
library(ggplot2)
library(ggpubr)
library(ggrepel)
...
otu = read.csv('species_data.csv', head = T, row.names=1)           #读取物种相对丰度表
otu <- data.frame(t(otu))

# 分组数据
# group data
matadata <- read.table(paste("group_data.txt",sep=""), header=T, row.names=1, sep="\t", comment.char="")
otu = otu[rownames(otu) %in% rownames(matadata), ]

# 读取环境数据
# confounding factors
env = read.csv('c_index_species_new4.csv', header = T, row.names=1)
env = na.omit(env)
...
distance_mat = read.table(paste("Species_beta_diversity2.txt",sep=""), header=T, row.names=1, sep="\t", comment.char="")
...
CPA_data$group = sub("[0-9]","_", CPA_data$group)
CPA_data$group = gsub("[0-9]","", CPA_data$group)
CPA_data$group = gsub("R_","R_H", CPA_data$group)
CPA_data$group = gsub("_A","", CPA_data$group)
CPA_data$group = gsub("_B","", CPA_data$group)
CPA_data$group = gsub("_C","", CPA_data$group)
CPA_data$group = gsub("_D","", CPA_data$group)
CPA_data$group = gsub("_F","", CPA_data$group)
CPA_data$group = gsub("_V","", CPA_data$group)
CPA_data$group = gsub("R_","", CPA_data$group)
...
#ggsave(paste("data/dbRDA_Species01.pdf",".pdf", sep=""), p1, width=149 * 1.5, height=80 * 1.5, unit='mm')
p1
...
##读取数据
#读入物种数据，以细菌 OTU 水平丰度表为例
otu = read.csv('pathway_unstrafied4.csv', head = T, row.names=1)           #读取物种相对丰度表
otu <- data.frame(t(otu))
...
# Pathways
distance_mat = read.table(paste("bray_curtis_path3.txt",sep=""), header=T, row.names=1, sep="\t", comment.char="")
metadata2 = t(matadata)
distance_mat2 = distance_mat[rownames(distance_mat) %in% rownames(matadata), ]
distance_mat3 = distance_mat2[, colnames(distance_mat2) %in% colnames(metadata2)]
distance_mat = distance_mat3
...
#ggsave(paste("data/dbRDA_HP_new_PATHs01.pdf",".pdf", sep=""), p2, width=149 * 1.5, height=80 * 1.5, unit='mm')
p2
...
#导出每个环境因子的解释度
#Export the explanation of each environmental factor
write.csv(cap.hp$Hier.part, 'env_effect_HP_new_paths_04.csv')
...
ggsave(paste("dbRDA_species_pathways",".pdf", sep=""), p_all4, width=129 * 1.5, height=60 * 1.5, unit='mm')

```



beta多样性因子variance explained percent热图
Beta diversity factor variance explained percent heat map

```{r variance explained percent heatmap, include=TRUE}
...
allnew <- read.csv('data_heat_4.csv', header = T)
...
p1 <- ggplot(allnew, aes(variable,names, fill = deviance_explained)) +
...
ggsave(paste("dbRDA_HP_heat_n01_only_sex_age_new0318_3",".pdf", sep=""), p1, width=79 * 1.5, height=100 * 1.5, unit='mm')
p1
