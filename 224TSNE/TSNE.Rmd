...
### tSNE数据降维及绘图
TSNE data dimensionality reduction and plotting

> 本节作者：白德凤：中国农业科学院深圳基因组研究所
>Author of this section: Bai Defeng: Shenzhen Institute of Genomics, Chinese Academy of Agricultural Sciences
>
> 版本1.0.0，更新日期：2024年4月17日
>Version 1.0.0, updated on April 17, 2024
...
美国纪念斯隆凯瑟琳癌症研究中心的Marcel R.M. van den Brink团队2023年发表在Cell上的论文(Nguyen et al., 2023)，研究药物、肠道微生物组和癌症患者死亡之间的关系。我们以此图为例进行讲解。

The paper published by Marcel R.M. van den Brink's team at the Sloan Katherine Cancer Research Center in the United States in 2023 in Cell (Nguyen et al., 2023) investigates the relationship between drugs, gut microbiota, and cancer patient mortality. We will use this diagram as an example to explain.
...
#### tSNE绘图实战
tSNE using R software

#### tSNE投影肠道微生物群的组成空间图
Spatial map of the composition of intestinal microbiota projected by tSNE

##### 导入需要的软件包
Load packages

```{r setup1, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, dev = c('png', 'pdf')) #设置全局环境(Set global environment)
library(tidyverse) #集合包用于制图和对数据进行操作(Collection packages are used for drawing and manipulating data)
library(pheatmap) #用于制作热图的软件包(Software package for creating heat maps)
library(reshape2) #实现长、短数据表格转换的软件包(A software package for converting long and short data tables)
library(igraph) #igraph软件包可以进行网络可视化(The igrath software package can perform network visualization)
```
...
```{r load_data}
# 整体：健康组+疾病组
#Overall: Health group+Disease group
#丰度数据表和metadata
#Abundance data table and metadata
design = read.table("data/design.txt", header=T, row.names=1, sep="\t")
design$group = design$groupID
otufile = "data/sum_s2.txt"
...
##### 鉴定最丰富的属
Identify the most diverse genera

```{r dominant_taxa, include=TRUE}
library(dplyr)
library(Rtsne)
#整体(Overall)
#计算得到每个样本中每个微生物属的相对丰富
#Calculate the relative abundance of each microbial genus in each sample
otutab <- t(otutab)
otutab <- otutab[-1, ]
sample = rownames(otutab)
otutab <- cbind(sample, otutab)
otutab <- as.data.frame(otutab)
genus_abundance_tbl = otutab %>%
  filter(sample %in% design$Samples)
#得到每个样本优势微生物属的数据
#Obtain data on dominant microbial genera for each sample
dominant_genus = colnames(genus_abundance_tbl[,-1])[apply(genus_abundance_tbl[,-1], 1, which.max)]
# 得到每个样本优势微生物属的数据
#Obtain data on dominant microbial genera for each sample
design$dominant_genus = dominant_genus
#计算tSNE坐标
#Calculate the tSNE coordinates
...
#利用Rtsne计算tSNE坐标
#Using Rtsne to Calculate tSNE Coordinates
tsne_out <- Rtsne(bc_data, pca=FALSE, perplexity=45, theta=0.1, max_iter = 3000)
#Y中存储了画图坐标
#Y stores drawing coordinates
stnes = tsne_out$Y
...
genus_abundance_tbl2 = otutab2 %>%
  filter(sample2 %in% design2$Samples)
#得到每个样本优势微生物属的数据
#Obtain data on dominant microbial genera for each sample
dominant_genus2 = colnames(genus_abundance_tbl2[,-1])[apply(genus_abundance_tbl2[,-1], 1, which.max)]
# 得到每个样本优势微生物属的数据
#Obtain data on dominant microbial genera for each sample
design2$dominant_genus2 = dominant_genus2
#计算tSNE坐标
#Calculate the tSNE coordinates
#Load bray-curtail Beta diversity Matrix
...
#利用Rtsne计算tSNE坐标
#Using Rtsne to Calculate tSNE Coordinates
tsne_out2 <- Rtsne(bc_data2, pca=FALSE, perplexity=45, theta=0.1, max_iter = 3000)
#Y中存储了画图坐标
#Y stores drawing coordinates
stnes2 = tsne_out2$Y
...
genus_abundance_tbl3 = otutab3 %>%
  filter(sample3 %in% design3$Samples)
#得到每个样本优势微生物属的数据
#Obtain data on dominant microbial genera for each sample
dominant_genus3 = colnames(genus_abundance_tbl3[,-1])[apply(genus_abundance_tbl3[,-1], 1, which.max)]
#dominant_genus3
# 得到每个样本优势微生物属的数据
#Obtain data on dominant microbial genera for each sample
design3$dominant_genus3 = dominant_genus3
#计算tSNE坐标
#Calculate the tSNE coordinates
#Load bray-curtail Beta diversity Matrix
...
#利用Rtsne计算tSNE坐标
#Using Rtsne to Calculate tSNE Coordinates
tsne_out3 <- Rtsne(bc_data3, pca=FALSE, perplexity=45, theta=0.1, max_iter = 3000)
#Y中存储了画图坐标
#Y stores drawing coordinates
stnes3 = tsne_out3$Y
...
#整体：健康组+疾病组
#Overall: Health group+Disease group
p1 <- ggplot(design, aes(x=stnes[,2],y=stnes[,1],col=factor(dominant_genus))) +
  geom_point(size = 3) +
...
#健康组(Healthy)
p2 <- ggplot(design2, aes(x=stnes2[,2],y=stnes2[,1],col=factor(dominant_genus2))) +
  geom_point(size = 3) +
  #scale_color_manual(name = "Legend", values=color_scheme_genus_vector) +
  #color = ""+
  theme_classic() +
...
#疾病组(Patients)
p3 <- ggplot(design3, aes(x=stnes3[,2],y=stnes3[,1],col=factor(dominant_genus3))) +
  geom_point(size = 3) +
  #scale_color_manual(name = "Legend", values=color_scheme_genus_vector) +
  #color = ""+
  theme_classic() +
...
##### 通过kmeans方法确定最佳聚类数量
Determine the optimal number of clusters using the kmeans method

```{r kmeans cluster, echo=TRUE}
...
##### DMM模型方法进行聚类群分析
Cluster analysis using DMM model method

```{r DMM cluster, echo=TRUE}
...
#参考http://www.360doc.com/content/22/0326/22/73394596_1023477764.shtml
#安装和导入DMM模型聚类分析需要的软件包
#Install and import the software package required for DMM model clustering analysis
library(BiocManager)
...
# tax <- tax[,c('clade_name',metadata$sample)]
...
#只挑选出核心群进行分析
#Only select the core group for analysis
...
#删除不想要的OTU或群
#Delete unwanted OTUs or groups
...
write.csv(ass, file="results/DMM_3clusters_L6.csv")

for(k in seq(ncol(fitted(best)))){
  #melt是数据整合函数，将宽格式转化为长格式
  #Melt is a data integration function that converts wide format to long format
  d<-melt(fitted(best))
  #数据重塑后列名设置
  #Column name settings after data reshaping
  colnames(d)<-c("OTU","cluster","value")
  d<-subset(d, cluster==k) %>%
    #按照assignment strength升序排列
    #Arrange in ascending order according to assignment strength
    arrange(value) %>%
    #mutate函数在数据框中添加新变量，unique函数去除重复值提取唯一值，并将对应OTU转换为因子，只展示最重要的
    #The mutate function adds new variables to the data box, the unique function removes duplicate values, extracts unique values, and converts the corresponding OTU into factors, only displaying the most important ones
    mutate(OTU = factor(OTU, levels = unique(OTU))) %>%
    #过滤OTUvalue值大于四分位数的
    #Filter OTUvalue values greater than quartiles
